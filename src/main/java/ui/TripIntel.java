...
public class TripIntel extends HomeUI {
        ...
        for (int i = 0; i < result.size() - 1; i++) {
            ...
            switch (mode) {
                ...
                case "SAME_TRIP", "TRANSFER" -> {
                    SegmentBoxResult tripSegment = createTripSegmentBox(result, i);
                    hBox = tripSegment.hBox;
                    i = tripSegment.newIndex; // Update index to skip processed segments
                }
            }
    ...
    private record SegmentBoxResult(HBox hBox, int newIndex) {}

    private SegmentBoxResult createTripSegmentBox(List<Node> result, int i) {
        ...
        return new SegmentBoxResult(hBox, j - 1);
    }
}